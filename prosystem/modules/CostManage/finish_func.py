...
def getFinishCost(request):
    ...
    try:
        plans_list = FinishedProduct.query.filter(FinishedProduct.is_addp ==False,
    ...
    for plans in plans_list:
        ...
        if boms_sons_list.count() == 0:
            plan = AllCostInfo(
                ...
                cate = "产成品", year = current_year,type = "费用"
            )
            ...
            db.session.commit()
        try:
            FinishedProduct.query.filter_by(id=plans.id).update({"is_addp":True})
            db.session.commit()
        except Exception as e:
            db.session.rollback()
            current_app.logger.error(e)
    ...
    return render_template('six/sixfour.html',data=data)