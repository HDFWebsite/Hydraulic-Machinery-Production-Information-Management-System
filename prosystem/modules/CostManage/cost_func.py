...
def getFinishCost(request):
    ...
    try:
        plans_list = MaterialCost.query.filter(MaterialCost.is_addp ==False
    ...
    for plans in plans_list:
        ...
        if boms_sons_list.count() == 0:
            plan = AllCostInfo(
                ...
                cate = plans.son_cate, year = current_year,type = "费用"
            )
            plan = other_func.getAllneedModel1TP(current_month,plan,plans)
            db.session.add(plan)
        else:
            plan = boms_sons_list.first()
            plan = other_func.getAllneedModel2TP(current_month, plan, plans)
            db.session.commit()
        try:
            MaterialCost.query.filter_by(id=plans.id).update({"is_addp":True})
            db.session.commit()
        except Exception as e:
            db.session.rollback()
            current_app.logger.error(e)
    ...
    try:
        plans_list = SemifinishedCost.query.filter(SemifinishedCost.is_addp ==False
    ...
    for plans in plans_list:
        ...
        if boms_sons_list.count() == 0:
            ...
            db.session.commit()
        try:
            SemifinishedCost.query.filter_by(id=plans.id).update({"is_addp":True})
            db.session.commit()
        except Exception as e:
            db.session.rollback()
            current_app.logger.error(e)
    ...
    return render_template('six/sixone.html',data=data)