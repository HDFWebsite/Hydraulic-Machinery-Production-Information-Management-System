...
def getNeedPlan(request):
    year = request.args.get('year', datetime.now().strftime("%Y"))
    try:
        plans_list = MaterialRequire.query.filter(MaterialRequire.is_addo ==False).order_by(MaterialRequire.id.asc()).all()
    except Exception as e:
        db.session.commit()
        current_app.logger.error(e)
        return jsonify(errno=RET.DATAERR, errmsg="数据查询失败！")
    for plans in plans_list:
        current_year = plans.build_time.strftime("%Y")
        current_month = plans.build_time.strftime("month"+"%m")#month04

        boms_sons_list = AllNeedInfo.query.filter(AllNeedInfo.main_id == plans.require_id,
               AllNeedInfo.year == current_year,AllNeedInfo.type=="需求")
        if boms_sons_list.count() == 0:
            plan = AllNeedInfo(
                main_id=plans.require_id,main_name=plans.require_name,unit = plans.unit,
                cate = plans.cate, year = current_year,type = "需求"
            )
            ...
            db.session.commit()
        try:
            MaterialRequire.query.filter_by(id=plans.id).update({"is_addo":True})
            db.session.commit()
        except Exception as e:
            db.session.rollback()
            current_app.logger.error(e)
    ...
    try:
        boms_list = AllNeedInfo.query.filter(AllNeedInfo.cate!="产成品",
            AllNeedInfo.type=="需求" , AllNeedInfo.year == year).order_by(AllNeedInfo.cate.asc())
    except Exception as e:
        current_app.logger.error(e)
        return jsonify(errno=RET.DATAERR, errmsg="数据查询失败！")
    data = other_func.get_boms_list(boms_list,year)

    return render_template('five/fivetwo.html',data=data)