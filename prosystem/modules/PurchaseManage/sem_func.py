...
def getSemData(request):
    ...
    try:
        plans_list = SemifinishedProduct.query.filter(SemifinishedProduct.is_add ==False
    ...
    for plans in plans_list:
        ...
        if boms_sons_list.count() == 0:
            plan = AllCostInfo(
                ...
                cate ="自制半成品", year = current_year,type = "消耗量"
            )
            ...
            db.session.commit()
        try:
            SemifinishedProduct.query.filter_by(id=plans.id).update({"is_add":True})
            db.session.commit()
        except Exception as e:
            db.session.rollback()
            current_app.logger.error(e)
    ...
    return render_template('five/fivefive.html',data=data)